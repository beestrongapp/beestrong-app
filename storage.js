...
let S={month:new Date().getMonth(),year:new Date().getFullYear(),selectedDate:null,templates:[],workouts:{},measurements:{},activeWorkout:null,timerSecs:0,timerEnd:null,timerInterval:null,modal:null,loaded:false,isPro:false,coachMode:false,coachVisible:ld('bs-coach-visible-v1',true),programs:[],clients:[],pendingInvites:[],pendingClientRequests:[],user:null,defaultRest:90,units:'metric',layoutMode:localStorage.getItem('bs-layout-mode-v1')||'standard',goal:localStorage.getItem('bs-goal-v1')||'strength',level:localStorage.getItem('bs-level-v1')||'beginner',weekPlan:{},exerciseNotes:ld('bs-ex-notes-v1',{}),shareRecords:localStorage.getItem('bs-share-records-v1')==='1'};
...
function loadData(){
  ...
  S.coachVisible=ld('bs-coach-visible-v1',true);
  ...
  S.shareRecords=localStorage.getItem('bs-share-records-v1')==='1';
  ...
  if(!localStorage.getItem('bs-onboarded-v1'))localStorage.setItem('bs-onboarded-v1','1');
}
...
function saveAll(){
  sv('bs-tpl-v4',S.templates);sv('bs-wo-v4',S.workouts);sv('bs-meas-v1',S.measurements);sv('bs-ispro-v1',S.isPro);sv('bs-coach-v1',S.coachMode);sv('bs-coach-visible-v1',S.coachVisible!==false);sv('bs-programs-v1',S.programs);sv('bs-clients-v1',S.clients);sv('bs-default-rest-v1',S.defaultRest);sv('bs-units-v1',S.units);sv('bs-week-plan-v1',S.weekPlan||{});sv('bs-ex-notes-v1',S.exerciseNotes||{});localStorage.setItem('bs-share-records-v1',S.shareRecords?'1':'0');if(S.layoutMode)localStorage.setItem('bs-layout-mode-v1',S.layoutMode);if(S.goal)localStorage.setItem('bs-goal-v1',S.goal);if(S.level)localStorage.setItem('bs-level-v1',S.level);
